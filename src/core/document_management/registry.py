...
class DocumentRegistry:
    ...
    def register(
        ...
        Returns:
            文档ID
        """
        # ========== 并发安全：重新加载最新的registry ==========
        # 防止多个后台任务同时写入时互相覆盖
        logger.debug(f"🔄 [并发安全] 重新加载最新的registry以避免竞争条件")
        self._load()
    ...
    def update_metadata(self, doc_id: str, metadata_key: str, metadata_value: Any) -> bool:
        """
        安全地更新文档元数据（并发安全）
        ...
        # 更新元数据
        self._registry[doc_id][metadata_key] = metadata_value

        # 保存
        self._save()

        logger.info(f"✅ 更新文档元数据: {doc_id} - {metadata_key}")
        return True
