...
- **AnswerAgent**: 问答代理，负责意图分析、答案生成、对话管理、工具调用
- **RetrievalAgent**: 检索代理，负责语义检索和上下文组装
- **SearchAgent**: 网络搜索代理，负责搜索引擎检索和 URL 内容分析
...
- **智能工具系统**:
  - 📎 文档检索工具（retrieve_documents）- 从已索引文档中检索相关内容
  - 🌐 网络搜索工具（search_web）- 搜索互联网获取最新信息或分析指定 URL
  - 用户可在聊天界面通过按钮启用/禁用工具
  - 支持工具组合使用（文档+网络搜索同时启用）
- **意图识别**: 自动判断是否需要检索文档
...
- 时间戳显示（年/月/日 时:分:秒）
- **智能滚动与导航**：
  - 🎯 智能滚动：进度更新时仅在用户位于底部时自动滚动，不打断历史消息查看
  - ⬆⬇ 快捷导航按钮：快速跳转到最早消息或最新回复
  - 💡 新内容提醒：用户查看历史时，新回复到达会触发动画提醒和未读计数徽章
  - 📍 精准定位：点击"查看最新内容"直接跳转到最新回复顶部（而非底部）
- 会话持久化存储（三种模式独立管理）
- 清空历史功能（同时清空文件和内存）
- 显示选中文档和相似度评分
...
│   │   │   ├── tools_config.py    # 工具配置
│   │   │   └── __init__.py
│   │   └── search/                # SearchAgent（网络搜索代理）
│   │       ├── agent.py           # 搜索代理实现
│   │       ├── nodes.py           # 工作流节点
│   │       ├── tools.py           # 搜索和爬取工具
│   │       ├── state.py           # 状态定义（TypedDict）
│   │       ├── prompts.py         # 搜索专用提示词
│   │       ├── utils.py           # MCP 客户端和辅助函数
│   │       ├── README.md          # SearchAgent 使用文档
│   │       └── __init__.py
│   ├── core/                      # 核心功能
│   │   ├── llm/                   # LLM 抽象层
...
1. **Multi-Agent System** (src/agents/)
   ...
   - **AnswerAgent**: 智能问答工作流（ReAct 架构）
     - 工具规划（plan）→ 工具执行（execute）→ 结果评估（evaluate）→ 答案生成（generate）
     - 支持工具：文档检索（retrieve_documents）、网络搜索（search_web）
   - **RetrievalAgent**: 文档检索工作流
     - 语义检索 → 上下文组装 → 结果排序
   - **SearchAgent**: 网络搜索工作流
     - 搜索引擎检索（search）→ URL 选择 → 内容爬取 → 信息提取 → 答案生成
     - URL 内容分析（url_analysis）→ 内容爬取 → 大小评估 → 直接对话/索引处理
...
#### AnswerAgent 工作流（ReAct 循环）
```
用户提问 + enabled_tools
  → plan (工具规划 - 根据用户启用的工具构造调用)
  → execute_tools (并行执行工具调用)
...
```
搜索模式 (search):
  查询 → web_search (搜索引擎) → select_urls (选择相关URL)
      → scrape_content (爬取内容) → extract_info (提取信息)
      → format_answer (生成答案)

URL分析模式 (url_analysis):
  URL列表 → scrape_content (爬取内容) → evaluate_size (评估内容量)
      ├─ 小内容 → direct_chat (直接对话)
      └─ 大内容 → index_then_chat (索引后对话)
```
...
<details>
<parameter name="new_string"><b>📝 更新日志 | Changelog（点击展开）</b></summary>

### 2026-02-24 - 会话管理 UI 优化与 SearchAgent 修复
- 🎨 **会话管理弹窗集成**
  - ✅ 主页新增会话管理弹窗（不需要页面跳转）
  - ✅ 会话统计仪表板（总数、单文档、多文档、纯对话）
  - ✅ 实时搜索和过滤会话
  - ✅ 批量删除功能（并行处理）
  - ✅ 会话详情面板（从右侧滑入，完整对话历史）
  - ✅ URL hash 支持（`/#sessions` 自动打开弹窗）
  - ✅ 键盘快捷键（ESC 关闭、点击背景关闭）
- 📊 **会话卡片信息丰富化**
  - ✅ 显示会话模式标签（🎯单文档/🔀跨文档/✋手动选择）
  - ✅ 显示启用的工具（📚文档检索/🌐网络搜索）
  - ✅ 显示调用的 Agent（🔍检索代理/🌐搜索代理）
  - ✅ 增强时间显示（同时显示创建和更新时间）
  - ✅ 统一主页弹窗和数据管理页面的卡片样式
- 🤖 **Agent 调用记录系统**
  - ✅ 自动记录每条消息使用的 Agent（RetrievalAgent、SearchAgent）
  - ✅ 会话级别统计使用过的所有 Agent
  - ✅ 在卡片和详情面板中显示 Agent 使用情况
  - ✅ 黄色标签区分 Agent 信息（与工具标签颜色不同）
- 🐛 **SearchAgent 内容截断修复**
  - ✅ 修复 `format_answer` 方法的 2000 字符硬限制问题
  - ✅ 改为智能动态分配策略（总量最大 100000 字符）
  - ✅ 单个来源不再限制为 2000 字符，根据总长度动态分配
  - ✅ 支持长文本 URL 内容完整保存和分析（如长篇新闻、报告）
  - ✅ 优化日志输出，显示合并后的内容长度

### 2026-02-14 - SearchAgent 集成到 AnswerAgent
- 🌐 **网络搜索工具上线**
  - ✅ 将 SearchAgent 集成到 AnswerAgent 的工具系统
  - ✅ 在 `tools_config.py` 中启用 `search_web` 工具
  - ✅ 实现 `AnswerTools.search_web()` 方法，调用 SearchAgent
  - ✅ 添加 `search_agent` 实例到 AnswerAgent（懒加载）
  - ✅ 支持两种搜索模式：搜索引擎检索和 URL 内容分析
- 🔧 **工具调用架构**
  - ✅ AnswerAgent 采用 ReAct 循环：plan → execute → evaluate → generate
  - ✅ 用户通过 UI 按钮（🌐）启用/禁用网络搜索工具
  - ✅ 支持工具组合：文档检索 + 网络搜索同时使用
  - ✅ 进度回调集成，实时 UI 更新

### 2026-02-04 - 聊天界面用户体验优化
- 🎨 **智能滚动系统**
  - ✅ 进度更新时仅在用户位于底部时自动滚动，避免打断历史消息查看
  - ✅ 新增 `isNearBottom()` 判断逻辑（距离底部 100px 阈值）
  - ✅ 所有滚动操作改为智能滚动（`smartScrollToBottom()`）
- 🧭 **快捷导航按钮**
  - ✅ 新增 "⬆ 最早消息" 按钮：快速跳转到对话顶部
  - ✅ 新增 "⬇ 查看最新内容" 按钮：跳转到最新助手回复顶部（而非底部）
  - ✅ 按钮位于输入框下方，三种对话模式通用
  - ✅ 仅在用户不在底部时显示，自动隐藏/显示
- 💬 **新内容提醒系统**
  - ✅ 用户查看历史消息时，新回复到达触发视觉提醒
  - ✅ 按钮文本动态切换："查看最新内容" → "有新内容，点击查看"
  - ✅ 脉冲发光动画 + 箭头弹跳动画 + 红色未读计数徽章
  - ✅ 未读计数自动累加，点击按钮或滚回底部后重置
- 🎭 **视觉与交互优化**
  - ✅ 两个按钮不同的渐变配色（紫色 vs 蓝紫色）
  - ✅ 平滑动画效果（slideDown、pulseButton、pulseBadge、bounceIcon）
  - ✅ 按钮悬停浮起效果，点击反馈动画
  - ✅ 移除 chat-enhancer.js 中的重复滚动按钮，避免冲突
